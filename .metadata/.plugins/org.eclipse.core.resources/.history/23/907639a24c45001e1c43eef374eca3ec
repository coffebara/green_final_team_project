...
public interface CourseRepository extends JpaRepository<Course, Long>, QuerydslPredicateExecutor<Item>, CourseRepositoryCustom {
		...
		List<Course> findByCourseTitleOrCourseDec(String courseTitle, String itemDec);
	...
	//메서드에 대한 쿼리를 직접 정의
		@Query("select c from Course c where c.courseDec like " + "%:courseDec% order by c.coursePrice desc")
		List<Course> findByCourseDec(@Param("courseDec") String courseDec);

		
		
	//itemDetail 필드에 지정된 텍스트가 포함된 모든 아이템을 찾아내고, 그 결과를 가격 내림차순으로 정렬
		@Query(value = "select * from course c where c.courseDec like "
				+ "%:courseDec% order by c.coursePrice desc", nativeQuery = true)
		List<Course> findByCourseDecByNative(@Param("courseDec") String courseDec);
	}